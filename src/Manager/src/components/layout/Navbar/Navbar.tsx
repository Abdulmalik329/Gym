...
// TS6133: formatTime olib tashlandi (ishlatilmagani uchun)
...
const Navbar: React.FC<NavbarProps> = ({ toggleSidebar }) => {
  ...
  // TS6133: canSend ishlatildi
  const canSend = useMemo(() => {
    return (
      notifTitle.trim().length > 0 &&
      notifMessage.trim().length > 0 &&
      (audience === "ALL" || selectedUserIds.length > 0)
    );
  }, [notifTitle, notifMessage, audience, selectedUserIds]);
  ...
  const sendNotification = async () => {
    ...
    try {
      let finalUserIds =
        audience === "ALL" ? gymUsers.map((u) => u.id) : selectedUserIds;
  ...
  // TS6133: readAll ishlatildi (tugma qo'shildi)
  const readAll = async () => {
    if (isManager) return;
    try {
              ...
              <ModalBody>
                {tab === "INBOX" ? (
                  <Block>
                    ...
                    >
                      <InboxSearch style={{ flex: 1, marginRight: 10 }}>
                        <Input
                          placeholder="Search..."
                          value={notifSearch}
